...
[new_res,predicted_y] = ZeppelinStick(final_params, voxel,bvals,qhat);

% [new_res,predicted_y] = findGlobalMin(initial_point, voxel,qhat,bvals,h);
save('q132Zepp.mat','final_params','new_res');
newGraph = plotFit(predicted_y, voxel);
saveas(newGraph, 'Diagrams/q132-ZeppelinStickSSD-voxel1.png');
...
function [sumRes, resJ] = ZeppelinStick(x0, Avox, bvals,qhat)
...
function [finalParams, resnorm] = findGlobalMin(x0, Avox, qhat, bvals,h)
    ...
    % Now run the fitting
    [parameter_hat,RESNORM,~,~] = fminunc(@ZeppelinStick, x0, h, Avox, bvals,qhat);
    global_min = RESNORM;
...
%     normalised_start = x0 + 0.0000000000001;
    ...
    for i=1:num_iterations
        ...
        while(state == 1)
            try
                random_start = normrnd(x0, [0.15,1e-09,0.1,2*pi,2*pi,1e-09,1e-09]);
%               random_start = normalised_start .* random_modifier;
                [parameter_hat,new_res,~,~] = fminunc(@ZeppelinStick, random_start, h, Avox, bvals,qhat);
                state = 0;
            catch
            end
        end        
