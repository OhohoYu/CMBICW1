...
for i=1:3
    ...
    h = figure;
    plot(mcmc_confidence, [5,5],'r-d', 'LineWidth', 0.75);
    ylim([0 6]);
    hold on;
    plot(mcmc_std, [4,4],'k-d', 'LineWidth', 0.75);
    plot(sample_confidence, [3,3],'g-x', 'LineWidth',0.75);
    plot(sample_std, [2,2],'b-x', 'LineWidth', 0.75);
    plot(laplace_std(i,:), [1,1],'m-o', 'LineWidth', 0.75);
    legend('Confidence limits Parametric Bootstrap','Standard Deviation Parametric Bootstrap','Confidence limits MCMC','Standard Deviation MCMC','location','northoutside');
    hold off;
    saveas(h,filename);
end
